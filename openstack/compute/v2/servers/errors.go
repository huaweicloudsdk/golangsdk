...
func (e ErrNeitherImageIDNorImageNameProvided) Error() string {
	e.DefaultErrString = "One and only one of the image ID and the image name must be provided."
	return gophercloud.ParseClientError(e.ErrMissingInput.BaseError)
...
func (e ErrNeitherFlavorIDNorFlavorNameProvided) Error() string {
	e.DefaultErrString = "One and only one of the flavor ID and the flavor name must be provided."
	return gophercloud.ParseClientError(e.ErrMissingInput.BaseError)
...
func (e ErrNoClientProvidedForIDByName) Error() string {
	e.DefaultErrString = "A service client must be provided to find a resource ID by name"
	return gophercloud.ParseClientError(e.ErrMissingInput.BaseError)
	//return ""
}
